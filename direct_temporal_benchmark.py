...
def run(*, n_towers=1, horizon=3, n_tilts=3, seed=0, mobility_seed=0,
    ...
    hysteresis_traj = myopic_hysteresis_trajectory(problem)
    hysteresis = type("Result", (), {"config": problem.flatten(hysteresis_traj),
                                      "objective": problem.objective(hysteresis_traj),
                                      "seconds": 0.0,
                                      "evaluations": horizon * problem.n_tilts ** problem.n_sectors,
                                      "extra": {"policy": "one-step switching-aware rolling horizon"}})()
    checkpoint(_method_from_solver("myopic hysteresis", "classical", hysteresis,
                                   problem, best))
