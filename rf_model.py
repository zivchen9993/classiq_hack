...
class Network:
    ...
    def evaluate_config_dynamic_service(self, tilt_indices) -> dict:
        """Sensitivity check with serving-cell assignment recomputed per tilt.

        The optimization QUBO intentionally freezes service areas so its RF
        objective remains unary + pairwise.  This evaluator removes that
        approximation *only for validation*: each grid point is associated to
        the strongest sector under the candidate tilt configuration and SINR
        is then evaluated jointly.  It is therefore useful for checking whether
        the fixed-service-area approximation changes configuration rankings.

        Handover failure is deliberately not reported here because the main RF
        model defines handover edge sets and natural targets at the fixed
        reference association.  Re-defining those simultaneously would change
        two modelling assumptions at once and obscure the sensitivity test.
        """
        tilt_indices = np.asarray(tilt_indices, dtype=int)
        n_s = len(self.sectors)
        if tilt_indices.shape != (n_s,):
            raise ValueError(f"expected {n_s} tilt indices, got {tilt_indices.shape}")

        p_lin = np.stack([self.rx_lin[i, tilt_indices[i], :] for i in range(n_s)])
        p_db = np.stack([self.rx_dbm[i, tilt_indices[i], :] for i in range(n_s)])
        serving = np.argmax(p_db, axis=0)
        cols = np.arange(len(self.grid_xy))
        signal = p_lin[serving, cols]
        total = p_lin.sum(axis=0)
        sinr_lin = signal / (total - signal + self.noise_lin)
        sinr_db = 10.0 * np.log10(sinr_lin)
        spectral_eff = np.minimum(np.log2(1.0 + sinr_lin), 7.0)
        weights = self.demand
        covered = sinr_db >= 0.0

        fixed_serving = np.argmax(self.service_mask, axis=0)
        fixed_has_server = self.service_mask.any(axis=0)
        changed = fixed_has_server & (serving != fixed_serving)
        changed_weight = float(np.average(changed, weights=weights))
        ...
        return {
            ...
            "outage_pct": float(100.0 * np.average(~covered, weights=weights)),
            "coverage_pct": float(100.0 * np.average(covered, weights=weights)),
            "server_change_pct_vs_reference": 100.0 * changed_weight,
            "n_points": int(sinr_db.size),
        }
